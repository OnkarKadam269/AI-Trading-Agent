...
def run_backtest():
    ...
    for pair in pairs:
        print(f"--- Running METHOD 1 (Pessimistic High/Low Execution + $5 Comm) for {pair} ---")
            ...
            for trade in list(open_trades):
                ...
                if trade['type'] == 'BUY':
                    if current_low <= trade['sl']:
                        hit_sl, exit_price = True, current_low
                    if current_high >= trade['tp']:
                        hit_tp = True
                        if not hit_sl: exit_price = trade['tp']
                        
                elif trade['type'] == 'SELL':
                    if current_high >= trade['sl']:
                        hit_sl, exit_price = True, current_high
