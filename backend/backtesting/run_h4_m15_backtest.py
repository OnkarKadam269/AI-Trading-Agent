...
def run_backtest():
    ...
    df_m15_ta = calculate_ta(df_m15_ta)
    ...
    print("\n--- Phase 2: Dual Timeframe Execution (H4 -> M15) ---")
    ...
    SPREAD_POINTS = 0.60
    RISK_PCT = 0.01  # Increased risk to 1.0%
        ...
        if open_trade is None:
            ...
            if htf_adx > 20:
                ...
                if htf_gap > 0.1 * htf_atr and curr['close'] <= curr['bb_lower']:
                    ...
                    open_trade = {
                        ...
                        'balance_before': balance, 'htf_atr': htf_atr
                    }
            ...
            if open_trade['type'] == 'BUY':
                if curr['low'] <= open_trade['sl']:
                    ...
                    new_sl = curr['close'] - (1.5 * open_trade['htf_atr'])
    ...
    print(f"\nH4->M15 Results: Trades {trades}, WinRate {win_rate:.1f}%, Final Balance ${balance:.2f}")
    
    df_log = pd.DataFrame(all_trades_log)
    df_log.to_csv("XAUUSD_H4_M15_Dual_Backtest.csv", index=False)
