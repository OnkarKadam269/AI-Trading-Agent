...
def run_backtest():
    ...
    # 2. Resample to M30 (Execution and ATR Shield)
    ...
    # Dual Timeframe Execution
    print("\n--- Phase 2: Dual Risk Execution (H4 -> M30) ---")
    
    df_h4 = df_h4.set_index('time')
    df_m30 = df_m30.dropna().reset_index()
    df_m30['time_ist'] = df_m30['time'] + pd.Timedelta(hours=3.5)
    ...
    print(f"Simulating M30 execution on {len(df_m30)} candles...")
    for i in range(1, len(df_m30) - 1):
        curr = df_m30.iloc[i]
        nxt = df_m30.iloc[i+1]
    ...
    print(f"\nH4->M30 Results: Trades {trades}, WinRate {win_rate:.1f}%, Final Balance ${balance:.2f}")
    
    df_log = pd.DataFrame(all_trades_log)
    df_log.to_csv("XAUUSD_H4_M30_7Month_Backtest.csv", index=False)
